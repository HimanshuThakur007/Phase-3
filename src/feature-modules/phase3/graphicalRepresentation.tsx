...
//   console.log("salesBucketData", salesBucketData);
...
const InventoryDashboard: React.FC = () => {
  ...
  const themeClass = theme === "dark_mode" ? "dark" : "light";
  ...
  const total_sale_value = totals?.total_sale_value;
  const total_str_stk_sp_value = totals?.total_str_stk_sp_value;
  const total_branch = summaries?.length;
  ...
  // Add this near your other useMemo or data logic
  // const sanitizedSalesBucketData = useMemo(() => {
  //   if (!salesBucketData || !Array.isArray(salesBucketData)) return [];

  //   return salesBucketData
  //     .map((item: any) => ({
  //       ...item,
  //       // Clamp negative values to 0
  //       value: Math.max(0, item.value || 0),
  //       // Optional: also sanitize import/local if used in tooltip
  //       import: Math.max(0, item.import || 0),
  //       local: Math.max(0, item.local || 0),
  //     }))
  //     .filter((item: any) => item.value > 0);
  // }, [salesBucketData]);
  // console.log("Sanitized Sales Bucket Data:", sanitizedSalesBucketData);
  // const bgColor = theme === "dark_mode" ? "#1e293b" : "#fff";
  const textColor = theme === "dark_mode" ? "#f1f5f9" : "#1e293b";
  const borderColor = theme === "dark_mode" ? "#334155" : "#e2e8f0";
  const cardBg = theme === "dark_mode" ? "#1e293b" : "#fff";
  const tooltipBg = theme === "dark_mode" ? "#1e293b" : "#fff";
  const tooltipBorder = theme === "dark_mode" ? "#334155" : "#e2e8f0";

  return (
    <>
      <style>{`
        .dashboard-wrapper { min-height: 100vh; display: flex; flex-direction: column; background: ${cardBg}; color: ${textColor}; }
        .dashboard-header {
        ...
        .dashboard-body { flex: 1; overflow-y: auto; padding: 1rem; background: ${cardBg}; }
        ...
        .card-header {
          ...
          border-bottom: 1px solid ${
            theme === "dark_mode" ? "#334155" : "#f1f5f9"
          } !important;
        }
        .card-title { font-weight: 700; color: ${textColor}; margin-bottom: 0; font-size: 1.1rem; }

        .stat-value { font-size: 1.8rem; font-weight: 800; line-height: 1; color: inherit; }
        .stat-label { font-size: 0.875rem; font-weight: 600; opacity: 0.8; letter-spacing: 0.5px; }
          ...
          <div className="row align-items-center">
            ...
            <div className="col-lg-6 text-lg-end">
              ...
              <button
                ...
                onClick={() => navigate("/stock-comparison")}
              >
              ...
              {[
                ...
                return (
                  <div
                    key={i}
                    className="flex-grow-1"
                    style={{ minWidth: "220px", maxWidth: "295px" }}
                  >
                    <div
                      className="card kpi-card h-100 shadow-sm w-100"
                      style={{
          ...
          <div className="row g-4 mb-3">
            ...
            <div className="col-lg-6">
              <div className="card chart-card h-100">
                ...
                <div className="card-body p-3" style={{ minHeight: 400 }}>
                  <ResponsiveContainer width="100%" height={dynamicHeight}>
                    <BarChart
                      data={stockBucketData}
                      margin={{ top: 20, right: 30, left: 60, bottom: 10 }}
